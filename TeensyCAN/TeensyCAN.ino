...
//     }
    ...
    //     Can0.write(txmsg);
    //     while (!Can0.write(txmsg)) 
    //     {
    //       delay(1000); // 버퍼가 비어질 때까지 잠시 대기합니다.
    //     }
    //     speed--;
    //   }
    // }
...
volatile bool waitForNextSpeed = false;
CAN_message_t txmsg;
int speed = 500;  
...
{
  if(msg.id == 5 && msg.buf[0] == 0xF6) 
  {
    if(msg.buf[1] == 0x01) 
    { // 데이터가 0x01인 경우
      waitForNextSpeed = false; // 대기 상태를 해제
    } 
    else if (msg.buf[1] == 0x00) 
    { // 데이터가 0x00인 경우
      waitForNextSpeed = true; // 다음 속도 메시지를 기다림
    }
  }
}
...
void loop() 
{
  ...
  if(Serial.available()) 
  {
    ...
    if(input == '1') 
    {
      ...
      txmsg.len = 5;
      txmsg.buf[0] = 0xF6;
      txmsg.buf[1] = (speed >> 8) & 0xFF;
      txmsg.buf[2] = speed & 0xFF;
      txmsg.buf[3] = 3;
      txmsg.buf[4] = (txmsg.id + txmsg.buf[0] + txmsg.buf[1] + txmsg.buf[2] + txmsg.buf[3]) & 0xFF;        
      Serial.printf("%02X %02X %02X %02X %02X %02X\n", txmsg.id, txmsg.buf[0], txmsg.buf[1], txmsg.buf[2], txmsg.buf[3], txmsg.buf[4]);        
      Can0.write(txmsg);
    ...
    if(input == '2') 
    {
        ...
        while(speed >= 0) 
        {
            ...
            Serial.printf("Sending: %02X %02X %02X %02X %02X %02X\n", txmsg.id, txmsg.buf[0], txmsg.buf[1], txmsg.buf[2], txmsg.buf[3], txmsg.buf[4]);
            Can0.write(txmsg);

            //unsigned long delayTime = (256 - acc) * 50;
